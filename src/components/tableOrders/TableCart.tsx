...
const TableCart = ({ id, orderId }: any) => {
  const apiUrl = process.env.NEXT_PUBLIC_API_URL;
  const dispatch = useDispatch();
  const cartItems = useSelector((state: any) => state.cart.items);
  const [loading, setLoading] = useState(false);
  const [orderType, setOrderType] = useState<any>('dining');
  ...
  const handleOrder = async () => {
    ...
    try {
      ...
      // Prepare order details with userType and orderType
      const orderDetails = {
        userToken: token,
        cartItems,
        tableId: id,
        userType: 'User', // Add the userType to the order
        orderType: orderType, // Add the orderType to the order
      };
